...
ShellRoot {
    ...
    // dock sempre visível quando workspace atual está vazio
    property bool isWorkspaceEmpty: {
        if (!Hyprland || !Hyprland.focusedWorkspace) return false
        var fid = Hyprland.focusedWorkspace.id
        if (!root.dockClients || root.dockClients.length === 0) return true
        for (var i = 0; i < root.dockClients.length; i++) {
            if (root.dockClients[i].workspace === fid) return false
        }
        return true
    }
    ...
    // ── Battery hover tooltip (porcentagem) ─────────────────
    Variants {
        model: Quickshell.screens
        PanelWindow {
            property var modelData
            screen: modelData
            visible: root.batteryHovered && !root.controlCenterVisible
            WlrLayershell.layer: WlrLayer.Top
            WlrLayershell.exclusionMode: ExclusionMode.Ignore
            WlrLayershell.keyboardFocus: WlrKeyboardFocus.None
            anchors { top: true; right: true }
            margins { top: 44; right: 6 }
            implicitWidth: 170
            implicitHeight: 32
            color: "transparent"
            Rectangle {
                ...
                radius: 8
                border.color: root.colOverlay0
                border.width: 1
                Text {
                    anchors.centerIn: parent
                    text: root.batteryCapacity + "%" + (root.batteryCharging ? " 󱐋" : "") + " • " + root.batteryStatus
                    // fallback curto se status muito longo
                    color: root.colText
        ...
        PanelWindow {
            ...
            Rectangle {
                ...
                RowLayout {
                    ...
                    // battery "{icon}" apenas ícone na barra (hover mostra %)
                    Text {
                        id: battText
                        ...
                        MouseArea {
                            ...
                            hoverEnabled: true
                            cursorShape: Qt.PointingHandCursor
                            onEntered: root.batteryHovered = true
                            onExited: root.batteryHovered = false
                            onClicked: {
                                root.controlCenterVisible = !root.controlCenterVisible
                                if (root.controlCenterVisible) root.calendarVisible = false
                            }
    ...
    Variants {
        ...
        PanelWindow {
            property var modelData
            screen: modelData
            // dock aparece quando hover, ou sempre se workspace vazio (sem janelas)
            visible: root.dockVisible || root.dockHovered || root.isWorkspaceEmpty
            WlrLayershell.layer: WlrLayer.Top
            WlrLayershell.exclusionMode: ExclusionMode.Ignore
            WlrLayershell.keyboardFocus: WlrKeyboardFocus.None
            anchors { bottom: true }
            margins { bottom: 8 }
            // responsiva: largura ajusta ao conteúdo (sem espaço vazio)
            implicitWidth: dockLoader.implicitWidth > 0 ? dockLoader.implicitWidth + 32 : 112
            implicitHeight: 88
            color: "transparent"
            // auto-hide timer
            Timer {
            ...
            Rectangle {
                id: dockBg
                anchors.centerIn: parent
                implicitWidth: dockLoader.implicitWidth > 0 ? dockLoader.implicitWidth + 16 : 96
                implicitHeight: 80
                width: implicitWidth
